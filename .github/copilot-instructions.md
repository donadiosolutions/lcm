...
- Keep Codecov reporting in mutually exclusive read-only CI jobs that check out the exact report source with credentials disabled, consume only the fixed CI artifact, and never execute repository code. The trusted job must grant and use OIDC only for pushes and same-repository PRs (including Dependabot); the fork-PR job must omit OIDC permission and use tokenless uploads. Both jobs remain skipped for `merge_group`.
