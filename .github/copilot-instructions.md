...
- Standalone managed-marker scanning must use a linear, non-overlapping line walk that does not consume the next line's prefix. Match only a complete line with optional spaces/tabs around a marker, preserve `lineStart`/`lineEnd` semantics for block removal, and keep unmatched or inline marker text as user content. Recognized managed regions must remain byte-idempotent across repeated installs; ambiguous or malformed marker/header combinations must be preserved conservatively and may require a second reinstall to become byte-stable.
- Managed-block cleanup must discover recognized current and legacy candidate regions in one linear pass, remove their sorted maximal unions in one retained-segment pass, and pair same-marker LCM blocks only from adjacent compatible marker lines. Pair distinct legacy START/END markers with monotonic earliest-start/first-later-end pointers, merge overlapping or touching candidate ranges deterministically instead of discarding a later recognized overlap, and preserve unmatched or inline marker text outside those unions. Treat only a current marker followed by one or more exact `# Workflow Instruction` lines up to EOF or the next managed marker as a recoverable header-only partial block. One-run healing applies only to recognized current/legacy managed blocks, the maximal union of their overlapping or touching ranges, and that narrowly recognized header-only form; it must never delete arbitrary user-authored heading lines. Reinstalling a recognized duplicate mix must converge to one canonical generated block in one run and then remain byte-idempotent.
...
- PostgreSQL lease expiry and other time-dependent eligibility checks must lock the exact row first, then evaluate the database clock in a later statement that begins after the lock is held. `statement_timestamp()` in the lock-bearing statement is fixed before a row-lock wait and can admit work that expired while blocked.
- PostgreSQL timestamps persisted by a lock-bearing statement must reflect the post-wait database clock: use `clock_timestamp()` for that write or timestamp locked rows in a later statement. Keep indexable eligibility predicates on `statement_timestamp()` when their pre-wait view can only defer work, not admit duplicate processing.
- Restore temporary transaction-local PostgreSQL settings such as `lock_timeout` on every success, failure, timeout, and cancellation path without replacing the original operation error. Tests must prove that a caught failed attempt cannot ratchet the temporary value into later work in the same transaction.
- Serialize the complete read/set/work/restore lifecycle for temporary transaction-local PostgreSQL settings per live transaction executor. Serializing individual queries or locking one repository instance is insufficient because concurrent calls and separate repository wrappers can share the same transaction.
- Treat SQLSTATE `55P03` as retryable when it is the expected result of a bounded coordination advisory-lock timeout, but do not classify it globally: the same state can represent explicit fail-fast lock policy in other PostgreSQL domains. Keep cancellation and unrelated permanent lock failures non-retryable.
...
- When preserved PostgreSQL tags act as presence markers, use `EXISTS` or a deduplicated source so duplicate tags cannot multiply one logical event. Keep ordered first-reference tag selection separate and test duplicate markers.
...
- Descriptor or pathname identity verification followed by pathname mutation is racy: another process can replace the path after verification and before the mutation. Unless the platform supplies an atomic identity-bound primitive for the exact operation, preserve the path and fail closed without mutation. Require deterministic regressions that insert regular replacements, symlinks, and hardlinks immediately after the verified descriptor closes.
- Security evidence is trustworthy only after required descriptor cleanup succeeds. Never swallow `closeSync` or equivalent finalization failures and return authenticated evidence; make cleanup failure override the result to unsafe and test that mutation paths remain unreachable.
- Legacy service-manager discovery must enumerate all units before strict historical-name filtering; active-state filters can omit reloading, refreshing, activating, deactivating, maintenance, inactive, failed, future, or otherwise untrusted states and manufacture false absence. Classify every exact listed-but-not-authenticatable state fail closed, accept only explicit exact not-found evidence as disappearance, and never stop without validated loaded/active/running state and a positive matching manager PID.
- Keep pre-mutation authority distinct from post-mutation observation. A systemd transition must never authorize the stop that caused it: require exact loaded/active/running state, the candidate's positive PID, and a valid nonzero canonical `InvocationID` before issuing an exact stop. Carry that invocation witness in the candidate and require exact continuity on every later observation. Only after the authenticated stop succeeds may bounded read-only polling retry exact active/running identity or loaded/deactivating state with one exact allowlisted systemd stop/final substate and either the original PID or literal PID 0. PID 0 is retry evidence only, never identity or success. Require exact not-found for success; refuse missing/malformed/changed invocation witnesses, PID drift, other transitions, unexpected substates, manager failures, and timeouts.
...
- Doctor stale-config repair success must require the doctor's own post-restart authenticated health recheck in addition to `restartDaemon.connected`; if that recheck is null or throws, fail closed with `fixApplied: false`, `daemonHealthy: false`, preserved remediation state, and the safe stale-config restart guidance. Do not infer authenticated health from lifecycle success alone.
...
- Recursive `mkdirSync(..., { mode: 0o700 })` does not tighten an existing broader directory. Persistent private directories must use the repository `ensurePrivateDirectory` convention, and tests must cover chmod repair of a pre-existing broad directory.
...
- Provenance gates must bind each immutable reviewed ref to the signed merge
  checkpoint's second parent; never compare provenance to a moving shared
  planning ref.
- When an implementation plan promises retained RED/GREEN or stability
  evidence to downstream reviewers, include that evidence in the exact review
  package; transient worker commentary is not an auditable substitute.
...
- Keep `external-admission.yml` limited to authenticated DCO `check_run` events, exact canonical CI `workflow_run` events, and default-branch `external-admission-reconcile` repository dispatch; never add a pull-request lifecycle trigger to this write-capable workflow. Authenticate checks by application ID, slug, and exact name. Require DCO check suites to supply a non-null, non-empty head branch and reject GitHub's reserved `gh-readonly-queue/` branch prefix before concurrency can select the synthetic SHA or any legacy status can be written; the permissionless merge-group workflow owns that SHA. Accept CI wake-ups only for `event=pull_request`, `path=.github/workflows/ci.yml`, the exact repository, and the event head SHA; reject push and merge-group CI runs before a runner starts. Every eligible PR requires authenticated canonical `ci` plus DCO on the exact head SHA of one open, non-draft PR in the exact repository targeting protected `main` or protected `maintenance/<major>.<minor>.x`, with live base protection checked through the policy CLI.
- Treat admission as a stateless event reducer. Every accepted DCO, canonical CI, or recovery event revokes stale success and evaluates the latest exact-head snapshot before exiting; never add a runner-resident polling loop. Stale event and workflow-run IDs are wake-ups, not state authority. Revalidate base protection and pull-request eligibility, required checks, and CI-run provenance at the initial, current, and final snapshots before success. Invalid, ambiguous, terminally unsuccessful, or ineligible evidence must become terminal failure; incomplete current checks remain pending for a later trusted wake-up.
...
- Every admission path must validate the successful CI check's Actions run metadata: `event=pull_request`, `path=.github/workflows/ci.yml`, exact repository, and exact head SHA, with explicit `actions: read` permission for the run lookup. A successful aggregate `ci` check is not sufficient while its backing workflow run is transient; admit only terminal success and leave every documented nonterminal state pending.
- Every authenticated event with a valid commit SHA must publish a non-success external-admission status before checkout, PR association, or eligibility lookup, because GitHub may omit closed unmerged PRs from a commit's PR associations. Paginate and flatten every page of commit-associated PRs and check runs before deciding uniqueness or success. Admit only one open, non-draft PR in the exact repository targeting protected `main` or protected `maintenance/<major>.<minor>.x` at the exact event SHA. Read live base protection and immediately before success repeat PR uniqueness and eligibility, required-check evaluation, and CI-run provenance validation; incomplete current checks remain pending, while invalid, ambiguous, terminal, or ineligible evidence is terminal failure.
- Load the executable admission policy only from the trusted workflow revision using the pinned checkout action, a minimal sparse checkout, and `persist-credentials: false`; never execute policy from the PR head. The write-capable `workflow_run` path must never download CI artifacts or caches or execute PR-controlled content: the evaluator never executes PR content. The initial provider-retirement policy transition required one explicitly documented maintainer bootstrap after manual exact-head validation; subsequent changes return to the normal no-bypass flow.
...
- Required checks that gate eligible protected PR bases must keep their pull-request trigger branch filters in parity across every eligible base namespace, including `main` and `maintenance/**`; preserve each workflow's other triggers and security contracts when adding a namespace.
- Admission freshness rules must derive a lower bound from every accepted event before selecting check or run evidence, so older observations cannot restore success after newer accepted evidence is received.
- Keep eligibility evaluation pure and separate from live API preflight; distinguish terminal invalid or ineligible responses from transient provider failures so retries do not become definitive rejection, and do not document unmerged implementation details as current behavior.
...
- Keep CodeRabbit and Copilot out of required status checks and external-admission authentication. External admission authenticates only DCO and canonical CI for the exact eligible head.
