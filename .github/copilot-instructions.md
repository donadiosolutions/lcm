...
- Keep both external-admission workflows intact for their distinct phases. The trusted `pull_request_target`, provider `status`, and provider `check_run` paths in `external-admission.yml` must authenticate CodeRabbit by creator ID, login, and status context, and authenticate `codecov/patch` and DCO by application ID, slug, and check name. Require all three successes on the exact head SHA of an open, non-draft PR targeting `main`.
...
- Keep the merge-group admission split intentional: `external-admission-merge-group.yml` must retain its `merge_group` / `checks_requested` trigger and permissionless Actions job named `external-admission`; it does not publish a commit status. CodeRabbit, Codecov, and DCO remain PR-head admission gates in `external-admission.yml`, while CI, all required CodeQL categories, and both Socket checks must still validate the synthetic commit.
