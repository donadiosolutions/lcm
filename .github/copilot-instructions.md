...
- Managed-daemon executable search paths may trust exact absolute launcher/runtime directories outside the current project plus explicit global-install or bundled-runtime anchors; never add `npx`, any `node_modules` directory, current-project, or ambient `PATH` entries.
- Diagnostics for a reused managed daemon must inspect that verified process's effective environment before using a deterministic startup fallback.
