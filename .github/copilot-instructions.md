...
- Every authenticated provider event with a valid commit SHA must publish a non-success external-admission status before PR association or eligibility lookup, because GitHub may omit closed unmerged PRs from a commit's PR associations. Admit only one open, non-draft, main-targeting PR at the exact event SHA, and repeat that validation immediately before publishing success; missing or ambiguous associations remain non-successful.
