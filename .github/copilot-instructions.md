...
- Keep Codecov reporting in a separate read-only CI job that checks out the exact report source with credentials disabled, consumes only the fixed CI artifact, and never executes repository code. It must use OIDC for pushes and same-repository PRs (including Dependabot), use tokenless uploads for fork PRs, and remain skipped for `merge_group`.
