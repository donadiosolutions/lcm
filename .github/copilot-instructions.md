...
- Keep `external-admission.yml` limited to authenticated provider `check_run` events, exact canonical CI `workflow_run` events, and manual exact-SHA reconciliation; never add a pull-request lifecycle trigger to this write-capable workflow. Authenticate checks by application ID, slug, and exact name. Accept CI wake-ups only for `event=pull_request`, `path=.github/workflows/ci.yml`, the exact repository, and the event head SHA; reject push and merge-group CI runs before a runner starts. Coverable or trust-sensitive diffs require `Greptile Review` from Greptile plus DCO; other diffs require `ci` from the GitHub Actions app plus DCO. All required successes must be on the exact head SHA of an open, non-draft PR targeting `main`.
- Treat admission as a stateless event reducer. Provider and CI created, requested, rerequested, or in-progress events revoke stale success and stop before policy checkout; completion events evaluate one fresh snapshot and exit. Never add a runner-resident polling loop. Leave incomplete or transient evidence pending for the next authenticated provider event, canonical CI lifecycle event, or manual reconciliation.
