...
- PostgreSQL advisory-lock get-or-create transactions must explicitly establish `READ COMMITTED` before their first lock or read. A configured `REPEATABLE READ` default can otherwise retain a pre-lock snapshot and miss the winning transaction's insert after waiting.
