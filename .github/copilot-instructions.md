...
- Keep `external-admission.yml` limited to authenticated provider `check_run` events, exact canonical CI `workflow_run` events, and default-branch `external-admission-reconcile` repository dispatch; never add a pull-request lifecycle trigger to this write-capable workflow. Authenticate checks by application ID, slug, and exact name. Require provider check suites to supply a non-null, non-empty head branch and reject GitHub's reserved `gh-readonly-queue/` branch prefix before concurrency can select the synthetic SHA or any legacy status can be written; the permissionless merge-group workflow owns that SHA. Accept CI wake-ups only for `event=pull_request`, `path=.github/workflows/ci.yml`, the exact repository, and the event head SHA; reject push and merge-group CI runs before a runner starts. Coverable or trust-sensitive diffs from normal contributors require `Greptile Review` from Greptile plus DCO. Sensitive PRs use authenticated canonical `ci` plus DCO only for authoritative Dependabot provenance: exact `dependabot[bot]` login, `user.type=Bot`, non-empty `dependabot/` head ref, equal head/base repository names, explicit trusted-root `greptile.json` `excludeAuthors` match, and neither current nor previous filename equal to `greptile.json`. Missing or malformed provenance fails closed; `github-actions[bot]` and `changeset-release/main` require Greptile for sensitive changes. Coverage-neutral diffs use CI plus DCO. All required successes must be on the exact head SHA of an open, non-draft PR targeting `main`.
...
- Classify admission using the complete paginated PR-file response, require its flattened record count to exactly equal the pull request's authoritative `changed_files`, and audit both `filename` and `previous_filename`. Require Greptile for every file under `bin/`, `installer/`, or `src/` (including shipped prompt and connector assets and PostgreSQL migrations); every `scripts/**/*.mjs`; `.github/actions/`, `.github/codeql/`, `.github/workflows/`, and `.github/scripts/`; trusted root `greptile.json`; `package.json`, its lockfile, and Vitest or TypeScript configuration, except for qualifying authoritative Dependabot provenance. Keep tests, docs, Changesets metadata, and other nonsensitive metadata coverage-neutral. Match exclusions case-insensitively with only `*` and `?` glob syntax; `[`, `]`, and `!` remain literal, and malformed config must fail closed. Validate automation from the authoritative pull request user, head ref, head repository, and base repository, never from a title, commit message, actor string, or untrusted branch content. Every CI-backed path must validate the successful CI check's Actions run metadata: `event=pull_request`, `path=.github/workflows/ci.yml`, exact repository, and exact head SHA, with explicit `actions: read` permission for the run lookup. A successful aggregate `ci` check is not sufficient while its backing workflow run is transient; admit only terminal success and leave every documented nonterminal state pending.
- Every authenticated provider event with a valid commit SHA must publish a non-success external-admission status before checkout, PR association, or eligibility lookup, because GitHub may omit closed unmerged PRs from a commit's PR associations. Paginate and flatten every page of commit-associated PRs, changed files, and check runs before deciding uniqueness or success. Admit only one open, non-draft, main-targeting PR at the exact event SHA. Immediately before success, repeat PR uniqueness and eligibility, file classification, automation-author identity, required-check evaluation, and CI-run provenance validation; missing, changed, or ambiguous evidence remains non-successful.
- Load executable admission policy and root `greptile.json` only from the trusted workflow revision using the pinned checkout action, a minimal sparse checkout, and `persist-credentials: false`; never execute policy or read exclusion config from the PR head. The write-capable `workflow_run` path must never download CI artifacts or caches or execute PR-controlled content. Admission workflow, policy, or exclusion-config changes cannot self-admit new logic that is not yet on the default branch, so their initial rollout requires an explicitly documented one-time maintainer bootstrap before returning to the normal no-bypass flow.
...
- Keep CodeRabbit and Copilot out of required status checks and external-admission authentication. External admission authenticates only Greptile Review, DCO, or CI according to the file classification and exact authoritative automation-author identity.
