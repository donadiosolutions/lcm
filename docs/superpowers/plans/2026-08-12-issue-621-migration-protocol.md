...
| `rolling-back` | `publish-rollback` | `rolled-back` | authoritative readback | one `rollback` report and explicit rollback mode; the `returnPhase` sealed by `prepare-rollback` is retained |
