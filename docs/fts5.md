...
This procedure targets Node `v25.0.0`, the runtime floor this package declares.

```bash
cd ~/Projects
git clone --depth 1 --branch v25.0.0 https://github.com/nodejs/node.git node-fts5
cd node-fts5
```
...
ln -sfn ~/Projects/node-fts5/out/Release/node ~/Projects/node-fts5/bin/node-25.0.0
```

Use a basename like `node-25.0.0`, `node`, or `nodejs`. Names like
`node-v25.0.0-fts5` may not be recognized correctly by Claude Code's CLI/runtime parsing.

Verify the new runtime:

```bash
~/Projects/node-fts5/bin/node-25.0.0 --version
~/Projects/node-fts5/bin/node-25.0.0 --input-type=module - <<'NODE'
import { DatabaseSync } from 'node:sqlite';
const db = new DatabaseSync(':memory:');
db.exec("CREATE VIRTUAL TABLE t USING fts5(content)");
console.log("fts5: ok");
...
```bash
/usr/libexec/PlistBuddy -c 'Set :ProgramArguments:0 /Users/youruser/Projects/node-fts5/bin/node-25.0.0' \
...
```text
program = /Users/youruser/Projects/node-fts5/bin/node-25.0.0
```
...
```bash
/Users/youruser/Projects/node-fts5/bin/node-25.0.0 \
