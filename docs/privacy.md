...
- Inside a bracketed `file://` query wrapper, redaction follows the value that owns it. A wrapper holding only plain literal values stops claiming path context at a `|` separator, so `file://h?x=[value|later=\Users\bob\secret.db]` is left unchanged, while a `?` or `#` in that position still claims it. Any URL inside the wrapper, including one appearing after the separator, keeps the wrapper claiming path context, so `file://h?x=[value|https://e.test/t|name=\Users\SECRET\x]` still redacts the trailing value. Commas and semicolons never end that ownership, and rooted values redact regardless of separator: POSIX paths, UNC paths, and Windows drive roots still become `<path>`.
- After a nested `file://` child inside such a wrapper, an ordinary relative value is preserved byte-for-byte, so `file://h?x=[https://e.test/t&/Users/a/one&file:///Users/b/two&relative/path]` keeps `relative/path`. Established private forms in the same position still redact: absolute POSIX paths, root-relative Windows paths, UNC paths, drive roots, `word/Users/...` values, and `name=\Users\...` parameters.
- A rooted path following a nested public URL is redacted in more shapes than before. This includes URLs carrying brackets, such as an IPv6 authority or a bracketed path segment, quoted URLs in either quote style, and a Windows drive root such as `C:\` as the value that arms the handoff. Repeated separators inside one wrapper each redact their successor rather than only the first.
- A rooted path after a closing bracket is redacted, including after several closing brackets and in a second bracket group following a closed one. Relative values in the same position remain visible, and whitespace still ends this behavior.
- Returning from a nested `file://` child behaves the same for `|` as for `&`, so `file://h?x=[file:///Users/CHILD/x|later/Users/alice/secret.db]` redacts its private tail on the first pass rather than the second.
