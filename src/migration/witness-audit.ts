...
export const MIGRATION_WITNESS_AUDIT: readonly MigrationWitnessAuditEntry[] = Object.freeze([
  ...
  },
  {
    ...
    description: "Per-domain dependency-edge-set equality between source and destination in canonical identity terms (PortableRecord.dependencies, already computed by the existing canonicalisation path on both sides), read fresh inside the window every pass; no witness value persists in the body, only the outcome. Catches a child remapped to a valid but wrong parent -- the #623 P1 shape -- which a foreign-key constraint cannot see because the remapped reference still points at a real row. Round-2-review stated scope reduction, not silently assumed complete: this equality check does not independently re-verify summary-parent-links acyclicity (a structurally cyclic chain of otherwise-individually-correct edges would not be caught, since set equality alone cannot see cycle structure) or re-derive project-aliases normalisation from source paths (it compares the already-normalised identities PortableRecord.dependencies carries on both sides, the same mechanism every other domain uses, rather than a bespoke per-domain re-normalisation pass); see verify-generation.ts's module-level scope-note comment for the same statement in the driver's own source.",
    comparison: "compared-live",
  ...
  },
  {
    ...
    description: "The destination's ordered-listing repository read, compared against the source's own canonical (createdAt, identitySha256) ordering captured during step 1, grouped by truncated createdAt with title compared within each group as a canonically sorted multiset rather than positionally (round-4 candidate-review-4 P1). title is folded in (round-4 P2, kept in this form) because the destination-native conversationId that listConversations() orders same-createdAt ties by is a different identity space than the source's portable identitySha256 -- the same correlation problem the search probe has -- and comparing it directly would compare a value the other side can never produce; comparing the two orderings positionally instead reported a mismatch on a correctly copied project whenever those two independent tie-breaks disagreed, which is routine. Grouping and comparing as a multiset removes that false positive while still catching a substitution or replacement inside a tie group. It remains a probe, not a proof: two same-millisecond conversations sharing a title are indistinguishable, and so is a permutation within one tie group -- two same-second conversations with their titles swapped with each other -- since telling that apart needs the intended pairing, which needs a correlation this probe deliberately does not add.",
    comparison: "compared-live",
  ...
  },
  {
    ...
    description: "publicProbeCoverage's two entries: public-listing is unconditionally true (runOrderedListingProbe always executes before this vector is built, so it has no reachable not-run outcome); public-search is the plan-v4 step 5 lcm.search_v1 search probe -- PostgreSqlLexicalSearchRepository.searchMessages, run before the window through the real repository, walking a bounded pool of source message candidates starting at an index derived from sampleParameters.seedBasisSha256 until one candidate's own content produces a *confirmed* destination search result (a self-match, never a cross-engine comparison against SQLite's own search behaviour). Round-4 P1: confirmation means the candidate's source-space identitySha256 is correlated to the destination's native message_id through lcm.transfer_identities and that native key is checked against the actual search results -- a bare non-empty result is no longer sufficient, since a search path that ignores its query or returns a fixed unrelated result would otherwise pass every attempt. This borrows trust from one ledger row per candidate tried: a false pass now requires both a broken search and a transfer_identities row that happens to name exactly the message the broken search returned.",
    comparison: "compared-live-probe-coverage",
    consequence: "public-listing's entry is always true. For public-search: when a correlated self-match is confirmed, folds ran plus the candidate pool size and the chosen candidate's canonical ordinal into publicProbeSha256, and its entry is true; when every candidate in the pool is exhausted without a confirmed self-match -- whether because a candidate's identity had no correlating transfer_identities row (unattributable) or because a search ran but did not return that candidate's own native key (unconfirmed) -- folds the not-run reason into publicProbeSha256 instead, its entry is false, and activationEligible cannot be true this pass regardless of what the listing probe or any mismatch class found -- an attributed absence per the frozen absence rule, not a mismatch and not a silent pass",
