...
export function migrationCopyLimits(input: MigrationCopyLimitsInput) {
    const result = { maxRecords: input.maxRecords, maxBytes: input.maxBytes, leaseTtlMs: input.leaseTtlMs ?? 300000, maximumTransactionAttempts: input.maximumTransactionAttempts ?? 3 };
    for (const [value, min, max] of [[result.maxRecords, 1, 500], [result.maxBytes, 1, 150994944], [result.leaseTtlMs, 1000, 86400000], [result.maximumTransactionAttempts, 1, 10]]) {
    ...
    for (let attempt = 0; attempt < input.maximumTransactionAttempts; attempt++) {
        if (proofRequired) {
            ...
            try {
                ...
                else if (!(error instanceof PostgreSqlStorageOperationError) || !['40001', '40P01'].includes(error.sqlState ?? ''))
                    throw error;
            }
        }
    }
...
import { createPostgreSqlPortableDestination, admitPortableDestinationInTransaction, readPortableRunInTransaction, readPortableBatchReceiptInTransaction, applyPortableBatchInTransaction, verifyPortableDestinationComplete, completePortableDestinationInTransaction, readPortableCompletedRunInTransaction, type PostgreSqlPortableDestinationInput } from '../storage/postgresql/portable-destination.js';
...
export async function openMigrationCopyDestination(input: MigrationCopyDestinationInput) {
    ...
    const resource = { resourceType: 'migration-copy', resourceKey: 'copy', processId: input.ownerProcessId, operation: 'migration-copy' };
    const context = { domain: 'factory' as const, operation: 'migrationCopy', projectId: input.expectedIdentity.id, machineId: input.expectedIdentity.machineId };
    ...
    const serializeReadback = async <T>(read: (executor: PostgreSqlTransactionScopeExecutor) => Promise<T>): Promise<T> => runtime.transaction(async (executor) => {
        ...
        const locked = await executor.query({ text: 'SELECT 1 AS locked FROM lcm.fenced_leases WHERE project_id=$1 AND resource_type=$2 AND resource_key=$3 FOR UPDATE', values: [input.expectedIdentity.id, resource.resourceType, resource.resourceKey] }, context);
    ...
    const mutate = async <T>(operation: (executor: PostgreSqlTransactionScopeExecutor) => Promise<T>): Promise<T> => {
        ...
        lease = lease ? await owner.renewLease({ ...resource, fencingToken: lease.fencingToken, ttlMs: input.leaseTtlMs, signal: input.signal }) : null;
    ...
    try {
        ...
        const readBatch = (batch: PortableBatch) => serializeReadback(executor => readPortableBatchReceiptInTransaction(executor, writer, batch));
        return {
            async admit(requireExisting: boolean) {
                ...
                await settleMigrationCopyOperation({ maximumTransactionAttempts: input.maximumTransactionAttempts, reconnect,
                    mutate: () => mutate(executor => admitPortableDestinationInTransaction(executor, writer, manifest, token, input.signal)), readback: readRun });
            },
            readBatch,
            async applyBatch(batch: PortableBatch) {
                return settleMigrationCopyOperation({ maximumTransactionAttempts: input.maximumTransactionAttempts, reconnect,
            ...
            async complete(verification: PortableDestinationVerification) {
                await settleMigrationCopyOperation({ maximumTransactionAttempts: input.maximumTransactionAttempts, reconnect,
                    ...
                    readback: async () => await serializeReadback(executor => readPortableCompletedRunInTransaction(executor, writer, verification)) ? true : null });
            },
            async readCompleted(verification: PortableDestinationVerification) { return serializeReadback(executor => readPortableCompletedRunInTransaction(executor, writer, verification)); },
            async close() {
                let primary: unknown;
                try {
                    if (lease)
