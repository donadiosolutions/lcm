...
const MCP_ASSIGNMENT_PREFIX_PATTERN = /(["']?)\b(host|hostname|socket|server|password|passwd|pwd|user(?:name|\s+id)?|uid|token|secret|api[-_\s]?key|authorization)\1(\s*)[:=]\s*(?:(?:basic|bearer)\s+)?/giu;
const MAX_MCP_DELIMITED_VALUE_LENGTH = 256;

function findBoundedDelimitedValueEnd(value: string, start: number, delimiter: string): number | undefined {
  const limit = Math.min(value.length - 1, start + 1 + MAX_MCP_DELIMITED_VALUE_LENGTH);
  let index = start + 1;
  while (index <= limit) {
...
function redactDelimitedMcpAssignments(value: string, redactionKeyMarker: string, redactionSeparator: string): string {
  let result = "";
  let cursor = 0;
  for (const match of value.matchAll(MCP_ASSIGNMENT_PREFIX_PATTERN)) {
    ...
    const delimiter = value[valueStart];
    ...
    const valueEnd = findBoundedDelimitedValueEnd(value, valueStart, delimiter);
    result += value.slice(cursor, start);
    result += redactedAssignment;
    if (valueEnd === undefined) {
      // A malformed or overlong delimited value is not safe to scan incrementally.
...
function safeMcpError(err: unknown): string {
  ...
  const protectedRaw = redactDelimitedMcpAssignments(markerSafeRaw, redactionKeyMarker, redactionSeparator).replace(
    /\b(host|hostname|socket|server|password|passwd|pwd|user(?:name|\s+id)?|uid|token|secret|api[-_\s]?key|authorization)(\s*)[:=]\s*(?:(?:basic|bearer)\s+)?([^\s,;]+?)([.!?])?(?=\s|[,;]|$)/giu,
