...
extends PostgreSqlQueryExecutor {
  transaction<T>(
    ...
    options: CoordinationContext & Pick<PostgreSqlTransactionOptions, "transactionMode">,
  ): Promise<T>;
}
    ...
    const result = await this.coordinationExecutor.query<LeaseDiagnosticRow>({
      text: `SELECT project_id, resource_type, resource_key,
             ...
             ORDER BY
               ...
               resource_key
             LIMIT $3`,
      values: [this.projectId, POSTGRESQL_BACKEND_PUBLICATION_RESOURCE_TYPE, normalizedLimit],
    }, this.queryOptions(operation));
  ...
  > {
    ...
    const result = await this.coordinationExecutor.query<DiagnosticsRow>({
      text: `SELECT
             ...
             WHERE project_id = $1`,
      values: [this.projectId, POSTGRESQL_BACKEND_PUBLICATION_RESOURCE_TYPE],
    }, this.queryOptions(operation));
    const row = result.rows[0];
