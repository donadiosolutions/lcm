...
Primary keys and unique constraints create additional B-tree indexes. The 53
...
| Promoted memory and recall | `promoted_memories_active_order_idx` supports active/stale scans; partial `promoted_memories_source_summary_idx` uses a bounded source-summary SHA-256 candidate and requires the exact source text residual without imposing a foreign key; partial `promoted_memories_source_project_idx` supports active owner-scoped source filtering; partial `promoted_memories_content_sha256_idx` uses `(project_id, content_sha256, created_at DESC, memory_id DESC)` for a bounded owner-scoped exact-content candidate with deterministic newest-first canonical selection, requiring a residual raw content equality comparison to reject theoretical hash collisions; `promoted_memories_metadata_idx`, `promoted_memories_search_document_idx`, and `promoted_memories_content_trgm_idx` cover metadata and content search; `promoted_memory_tags_lookup_idx` uses `(project_id, tag_sha256, memory_id)` for bounded exact case-sensitive candidate lookup while `promoted_memory_tags_normalized_lookup_idx` uses `(project_id, normalized_tag_sha256, memory_id)` for bounded normalized candidates. Both require a residual exact text comparison to reject theoretical hash collisions. `promoted_memory_tags_search_document_idx` and `promoted_memory_tags_tag_trgm_idx` support tag-only lexical search; `recall_surfacing_memory_order_idx` and partial `recall_surfacing_session_order_idx` support recall aggregation. |
...
The 0006 snapshot covered 27 tables, 100 indexes, 204 constraints, 253 column
...
## Owner exact-content digest index

Migration `0007_promoted_content_digest.sql` adds a generated, stored
SHA-256 `bytea` column `content_sha256` to `lcm.promoted_memories` and a
partial btree index `promoted_memories_content_sha256_idx` on
`(project_id, content_sha256, created_at DESC, memory_id DESC) WHERE
archived_at IS NULL`. The generated column is populated for every existing
row during the `ALTER TABLE` rewrite, so there is no null-key window and no
separate backfill step. Applying this migration takes an exclusive schema
rewrite and index-build window proportional to the existing promoted-memory
row count; operators should expect that window on databases with a large
promoted-memory table.

The digest is exactly raw content equality: raw-equal content always implies
digest equality under this deterministic server-side hash, and the exact
lookup in `findExactContent` retains raw byte-equal content comparison as a
residual filter after the indexed digest candidate, rejecting the
theoretical SHA-256 collision. This mirrors the repository's existing
`tag_sha256`, `source_summary_id_sha256`, and `session_id_sha256`
candidate-index pattern. No SQLite schema or query change is required for
this migration: the SQLite adapter already performs direct raw-content
