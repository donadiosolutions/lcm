 ...
 * listCanonicalHeaders and the local portable index already hand back. One
 * query answers up to PORTABLE_LIMITS.maxBatchRecords locators instead of one
 * round trip per record, which is the per-record shape #1388 flags for
 * canonical portable-source scanning (eachHeader's header walk and
 * boundaryAt's row reads). A locator absent from the source table is simply
 * absent from the returned map; callers must treat that as
 * record-unrepresentable, the same way a null readCanonicalRow result is
 * handled today, not silently skip it.
 */
