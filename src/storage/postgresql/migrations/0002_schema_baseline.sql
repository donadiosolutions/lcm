...
CREATE TABLE lcm.projects (
  ...
  identity_key text NOT NULL UNIQUE CHECK (
    identity_key OPERATOR(pg_catalog.~) '^[a-f0-9]{64}$'
  ),
...
CREATE TABLE lcm.conversations (
  ...
  session_id_sha256 bytea GENERATED ALWAYS AS (
    public.digest(session_id, 'sha256')
  ) STORED,
  title text,
...
CREATE INDEX conversations_session_lookup_idx
  ON lcm.conversations (
    project_id, session_id_sha256, created_at DESC, conversation_id DESC
  );
...
CREATE TABLE lcm.native_transcripts (
  ...
  native_session_id_sha256 bytea GENERATED ALWAYS AS (
    public.digest(native_session_id, 'sha256')
  ) STORED,
...
CREATE INDEX native_transcripts_session_idx
  ON lcm.native_transcripts (
    project_id, native_session_id_sha256, observed_at, transcript_id
  );
...
CREATE TABLE lcm.summaries (
  summary_key uuid PRIMARY KEY DEFAULT uuidv7(),
  summary_id text NOT NULL DEFAULT uuidv7()::text,
  summary_id_sha256 bytea GENERATED ALWAYS AS (
    public.digest(summary_id, 'sha256')
  ) STORED,
  project_id uuid NOT NULL,
  conversation_id bigint NOT NULL,
  ...
  UNIQUE (project_id, conversation_id, summary_key),
  FOREIGN KEY (project_id, conversation_id)
    REFERENCES lcm.conversations(project_id, conversation_id) ON DELETE CASCADE,
  CHECK ((uuid_extract_version(summary_key) = 7) IS TRUE),
  CHECK (earliest_at IS NULL OR latest_at IS NULL OR earliest_at <= latest_at)
);

CREATE INDEX summaries_identity_lookup_idx
  ON lcm.summaries (project_id, summary_id_sha256, summary_key);
CREATE INDEX summaries_conversation_order_idx
  ON lcm.summaries (project_id, conversation_id, created_at, summary_key);
CREATE INDEX summaries_project_recent_idx
  ON lcm.summaries (project_id, created_at DESC, summary_key DESC);
...
CREATE FUNCTION lcm.enforce_summary_id_uniqueness()
RETURNS trigger
LANGUAGE plpgsql
...
BEGIN
  ...
  IF EXISTS (
    SELECT 1
    FROM lcm.summaries AS existing
    WHERE existing.project_id OPERATOR(pg_catalog.=) NEW.project_id
      AND existing.summary_id_sha256 OPERATOR(pg_catalog.=) requested_sha256
      AND existing.summary_id OPERATOR(pg_catalog.=) NEW.summary_id
      AND existing.summary_key OPERATOR(pg_catalog.<>) NEW.summary_key
  ) THEN
    RAISE EXCEPTION 'duplicate summary identifier in project'
      USING ERRCODE = 'unique_violation',
            CONSTRAINT = 'summaries_project_summary_id_key';
  END IF;
  RETURN NEW;
END
$function$;

CREATE TRIGGER summaries_enforce_summary_id_uniqueness
BEFORE INSERT OR UPDATE OF project_id, summary_id ON lcm.summaries
FOR EACH ROW EXECUTE FUNCTION lcm.enforce_summary_id_uniqueness();
ALTER TABLE lcm.summaries
  ENABLE ALWAYS TRIGGER summaries_enforce_summary_id_uniqueness;
...
CREATE TABLE lcm.summary_messages (
  ...
  PRIMARY KEY (project_id, summary_key, message_id),
  UNIQUE (project_id, summary_key, ordinal),
  FOREIGN KEY (project_id, conversation_id, summary_key)
    REFERENCES lcm.summaries(project_id, conversation_id, summary_key) ON DELETE CASCADE,
  FOREIGN KEY (project_id, conversation_id, message_id)
...
CREATE INDEX summary_messages_message_idx
  ON lcm.summary_messages (project_id, conversation_id, message_id, summary_key);
CREATE INDEX summary_messages_summary_idx
  ON lcm.summary_messages (project_id, conversation_id, summary_key);

CREATE TABLE lcm.summary_parents (
  project_id uuid NOT NULL,
  conversation_id bigint NOT NULL,
  summary_key uuid NOT NULL,
  parent_summary_key uuid NOT NULL,
  ordinal integer NOT NULL CHECK (ordinal >= 0),
  PRIMARY KEY (project_id, summary_key, parent_summary_key),
  UNIQUE (project_id, summary_key, ordinal),
  FOREIGN KEY (project_id, conversation_id, summary_key)
    REFERENCES lcm.summaries(project_id, conversation_id, summary_key) ON DELETE CASCADE,
  FOREIGN KEY (project_id, conversation_id, parent_summary_key)
    REFERENCES lcm.summaries(project_id, conversation_id, summary_key)
    ON DELETE NO ACTION DEFERRABLE INITIALLY DEFERRED,
...
CREATE INDEX summary_parents_parent_idx
  ON lcm.summary_parents (
    project_id, conversation_id, parent_summary_key, ordinal, summary_key
  );
CREATE INDEX summary_parents_summary_idx
  ON lcm.summary_parents (project_id, conversation_id, summary_key);
...
CREATE TABLE lcm.context_items (
  ...
  FOREIGN KEY (project_id, conversation_id, summary_key)
    ...
    ON DELETE NO ACTION DEFERRABLE INITIALLY DEFERRED,
  CHECK (
    (item_type = 'message' AND message_id IS NOT NULL AND summary_key IS NULL) OR
    (item_type = 'summary' AND summary_key IS NOT NULL AND message_id IS NULL)
  )
);
...
CREATE INDEX context_items_summary_idx
  ON lcm.context_items (project_id, conversation_id, summary_key)
...
ALTER TABLE lcm.large_files
  ENABLE ALWAYS TRIGGER large_files_enforce_file_id_uniqueness;
...
CREATE TABLE lcm.promoted_memories (
  ...
  source_summary_id_sha256 bytea GENERATED ALWAYS AS (
    public.digest(source_summary_id, 'sha256')
  ) STORED,
...
CREATE INDEX recall_surfacing_session_order_idx
  ON lcm.recall_surfacing (
    project_id, session_id_sha256, surfaced_at, surfacing_id
  )
...
CREATE TABLE lcm.session_ingest_log (
  ingest_key uuid PRIMARY KEY DEFAULT uuidv7(),
  project_id uuid NOT NULL REFERENCES lcm.projects(project_id) ON DELETE RESTRICT,
  session_id text NOT NULL,
  session_id_sha256 bytea GENERATED ALWAYS AS (
    public.digest(session_id, 'sha256')
  ...
  CHECK ((uuid_extract_version(ingest_key) = 7) IS TRUE)
);

CREATE INDEX session_ingest_log_identity_lookup_idx
  ON lcm.session_ingest_log (project_id, session_id_sha256, ingest_key);
CREATE INDEX session_ingest_log_completed_idx
  ON lcm.session_ingest_log (project_id, completed_at DESC, ingest_key);

CREATE FUNCTION lcm.enforce_session_ingest_id_uniqueness()
RETURNS trigger
LANGUAGE plpgsql
SET search_path = pg_catalog, public
AS $function$
...
BEGIN
  ...
  IF EXISTS (
    SELECT 1
    FROM lcm.session_ingest_log AS existing
    WHERE existing.project_id OPERATOR(pg_catalog.=) NEW.project_id
    ...
    RAISE EXCEPTION 'duplicate session ingest identifier in project'
      USING ERRCODE = 'unique_violation',
            CONSTRAINT = 'session_ingest_log_project_session_id_key';
  END IF;
  RETURN NEW;
END
$function$;
...
BEFORE INSERT OR UPDATE OF project_id, session_id ON lcm.session_ingest_log
FOR EACH ROW EXECUTE FUNCTION lcm.enforce_session_ingest_id_uniqueness();
ALTER TABLE lcm.session_ingest_log
  ENABLE ALWAYS TRIGGER session_ingest_log_enforce_session_id_uniqueness;
...
CREATE TABLE lcm.session_instructions (
  ...
  scope_hash text NOT NULL CHECK (
    scope_hash OPERATOR(pg_catalog.~) '^[a-f0-9]{64}$'
  ),
...
CREATE INDEX session_instructions_machine_idx
  ON lcm.session_instructions (machine_id, instruction_id);
...
REVOKE ALL PRIVILEGES ON FUNCTION lcm.enforce_session_ingest_id_uniqueness() FROM PUBLIC;
