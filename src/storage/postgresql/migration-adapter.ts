...
export class PostgreSqlMigrationAdapter {
  ...
  async schemaHistory(signal?: AbortSignal): Promise<PostgreSqlSchemaHistoryEntry[]> { const result = await this.runtime.query<{ migration_id: unknown; sha256: unknown }>({ text: "SELECT migration_id, sha256 FROM lcm.schema_migrations ORDER BY migration_id" }, options(this.identity, "migrationSchemaHistory", signal)); return result.rows.map((row) => ({ id: text(row.migration_id, "migration_id"), sha256: text(row.sha256, "sha256") })); }
