...
const HELP: Record<string, CommandHelp> = {
  ...
  config: {
    ...
    notes: "Stored reads normalize compatibility aliases without rewriting the file. Secret-like values are always masked, including with --effective. Successful writes are atomic, preserve unrelated keys, and require `lcm daemon restart` before a running daemon uses the new configuration.",
  },
