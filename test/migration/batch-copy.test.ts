...
const remote = vi.hoisted(() => ({
    ...
    receipts: new Map<string, { checkpoint: PortableBatch['checkpoint']; batchSha256: string }>(),
    empty: true,
}));
...
vi.mock('../../src/storage/postgresql/portable-destination.js', async original => ({
    ...
    probePostgreSqlPortableDestination: async () => ({
        destinationWitnessSha256: 'a'.repeat(64),
        identityFingerprintSha256: 'b'.repeat(64),
        nonIdentityDomainsEmpty: remote.empty,
        existingRun: remote.run,
    }),
}));
...
vi.mock('../../src/migration/copy-destination.js', async original => ({
    ...
    openMigrationCopyDestination: async (input: Record<string, any>) => {
        ...
        return {
            async admit(required: boolean) {
                if (required && !remote.run) throw new Error('missing');
                remote.run ??= {
                    runId: input.runId,
                    targetGenerationId: input.generationId,
                    manifestSha256: manifest.manifestSha256,
                    projectFingerprintSha256: 'b'.repeat(64),
                    state: 'active',
                };
            },
            async readBatch(batch: PortableBatch) {
                return remote.receipts.get(batch.checkpoint.checkpointSha256) ?? null;
            },
            async applyBatch(batch: PortableBatch) {
                const receipt = {
                    checkpoint: batch.checkpoint,
                    batchSha256: canonicalSha256([batch.domain, batch.checkpoint.checkpointSha256]),
                };
                remote.receipts.set(batch.checkpoint.checkpointSha256, receipt);
                return receipt;
            },
            verify: async () => ({ complete: true }),
            readCompleted: async () => remote.run?.state === 'completed',
            async complete() { remote.run!.state = 'completed'; },
            close: async () => {},
        };
    },
}));
...
async function prepared(multiple = false) {
        ...
        try {
            for (const session of ['one', 'two', 'three']) {
                db.prepare('INSERT INTO session_instruction_cache VALUES (?,?,?,?,?,?,?,?,?)').run(
                    fixture.local.id, canonicalSha256(session), 'codex', session, fixture.cwd, fixture.cwd,
    ...
    const input = {
        generationId: snapshot.artifact.generationId,
        homeDir: fixture.homeDir,
        settings: fixture.request.targetConfig.postgresql,
        expectedOwner: 'owner',
        expectedIdentity: {
            id,
            remoteProjectId: id,
            localProjectId: fixture.local.id,
            machineId: REGISTERED_MACHINE,
            canonical: fixture.cwd,
            selectedPath: fixture.cwd,
        },
    ...
    const witness = await inspectSqliteMigrationCopy({ ...input, destinationCapturedAt: at });
    const store = new MigrationManifestStore({ homeDir: fixture.homeDir });
    let journal = store.create(createMigrationManifest({
        generationId: input.generationId,
        source: witness.source,
        destination: witness.destination,
        parentGenerationId: null,
        preservedSourceGenerationId: input.generationId,
        createdAt: at,
    }));
    journal = store.update(input.generationId, journal.checksumSha256, current => beginMigrationEffect(current, {
        kind: 'verify-dry-run', effectId: 'dryrun', inputSha256: 'a'.repeat(64), startedAt: at,
    }));
    store.update(input.generationId, journal.checksumSha256, current => completeMigrationEffect(current, {
        effectId: 'dryrun', completedAt: at,
        report: { kind: 'dry-run', reportId: 'verified', reportSha256: 'b'.repeat(64), createdAt: at },
    }));
...
it('publishes all exact checkpoints and proves copied reruns', async () => {
    ...
    expect(remote.receipts.size).toBe(22);
...
it.each([
    'before-source', 'after-source', 'before-batch', 'before-begin',
    ...
    await expect(runSqliteMigrationCopy(input, {
...
it('refuses an unrelated populated target without a transfer run', async () => {
    ...
    expect(store.read(input.generationId).pendingEffect).toBeNull();
...
it.each(['runId', 'targetGenerationId', 'manifestSha256'] as const)('refuses conflicting immutable target %s', async field => {
    const { input } = await prepared();
    await runSqliteMigrationCopy(input);
    remote.run![field] = 'changed';
    await expect(runSqliteMigrationCopy(input)).rejects.toThrow();
});
...
it('requires the exact earlier receipt even for a locally copied journal', async () => {
    ...
    await expect(runSqliteMigrationCopy(input)).rejects.toThrow();
    remote.receipts.delete(first[0]);
    await expect(runSqliteMigrationCopy(input)).rejects.toThrow();
});
...
it('refuses missing remote admission after local copying has started', async () => {
    ...
    await expect(runSqliteMigrationCopy(input, {
        observe: async boundary => {
            if (boundary === 'after-checkpoint' && !stopped) {
                stopped = true;
                throw new Error('stop');
            }
        },
    })).rejects.toThrow();
    expect(store.read(input.generationId).phase).toBe('copying');
    remote.run = null;
    await expect(runSqliteMigrationCopy(input)).rejects.toThrow();
});
...
it('accepts only the exact legal CAS successor after a lost publication acknowledgment', async () => {
    const { input } = await prepared();
    const original = MigrationManifestStore.prototype.update;
    vi.spyOn(MigrationManifestStore.prototype, 'update').mockImplementationOnce(function (...args) {
...
it('leaves conflicting CAS publication unresolved before data mutation', async () => {
    const { input, store } = await prepared();
    vi.spyOn(MigrationManifestStore.prototype, 'update').mockImplementationOnce(() => {
...
it('refuses a pending effect with a different logical input before receipt recovery', async () => {
    const { input, store } = await prepared();
    const current = store.read(input.generationId);
    store.update(input.generationId, current.checksumSha256, value => beginMigrationEffect(value, {
        kind: 'copy-batch', effectId: 'different-effect', inputSha256: 'f'.repeat(64), startedAt: value.updatedAt,
    }));
    await expect(runSqliteMigrationCopy(input)).rejects.toThrow();
    expect(remote.receipts.size).toBe(0);
});
...
it('refuses a non-copy pending effect and cancels before source inspection', async () => {
    ...
    const current = store.read(input.generationId);
    store.update(input.generationId, current.checksumSha256, value => beginMigrationEffect(value, {
        kind: 'abort', effectId: 'abort', inputSha256: 'f'.repeat(64), startedAt: value.updatedAt,
    }));
    await expect(runSqliteMigrationCopy(input)).rejects.toThrow();
...
it('rejects unsupported caller configuration before opening a source or target', async () => {
    ...
    for (const patch of invalidPatches) {
        await expect(runSqliteMigrationCopy({ ...input, ...patch } as never)).rejects.toThrow();
    }
...
it('cancels immediately after an awaited observation without creating a pending effect', async () => {
    ...
    await expect(runSqliteMigrationCopy(
        { ...input, signal: controller.signal },
        { observe: async () => { controller.abort(); } },
    )).rejects.toThrow();
    expect(store.read(input.generationId).pendingEffect).toBeNull();
    expect(remote.run).toBeNull();
});
...
it.each(['inspect', 'copy'] as const)('preserves uncertain commit from the %s target boundary', async mode => {
    ...
    const operation = mode === 'inspect'
        ? inspectSqliteMigrationCopy({ ...input, destinationCapturedAt: '2026-09-14T03:00:00.000Z' })
        : runSqliteMigrationCopy(input);
...
it.each(['primary', 'success'] as const)('sanitizes inspection close failure while preserving %s outcome', async outcome => {
    const { input } = await prepared();
    const open = sourceApi.openMigrationCopySource;
    vi.spyOn(sourceApi, 'openMigrationCopySource').mockImplementationOnce(async args => {
        const source = await open(args);
        const close = source.stream.close;
        return {
            ...source,
            stream: { ...source.stream, close: async () => { await close(); throw new Error('private close payload'); } },
        };
    });
    const unknown = new PostgreSqlCommitOutcomeUnknownError({ domain: 'factory', operation: 'test' });
...
it('reauthenticates the source inside the destination transaction authority callback', async () => {
    const { input } = await prepared();
    const open = destinationApi.openMigrationCopyDestination;
    vi.spyOn(destinationApi, 'openMigrationCopyDestination').mockImplementationOnce(async args => {
        await args.withSourceAuthority(async reauthenticate => { await reauthenticate(); });
        return open(args);
    });
    expect((await runSqliteMigrationCopy(input)).phase).toBe('copied');
});
...
it.each(['source', 'destination'] as const)('refuses a changed %s witness before admission', async field => {
    const { input } = await prepared();
    const read = MigrationManifestStore.prototype.read;
    vi.spyOn(MigrationManifestStore.prototype, 'read').mockImplementationOnce(function (...args) {
        const current = read.apply(this, args);
        return { ...current, [field]: { ...current[field], contentSha256: 'e'.repeat(64) } };
    });
    await expect(runSqliteMigrationCopy(input)).rejects.toThrow();
    expect(remote.run).toBeNull();
});
...
it.each(['source', 'destination', 'both'] as const)('sanitizes %s cleanup failure after a completed copy', async failure => {
    const { input } = await prepared();
    const sourceOpen = sourceApi.openMigrationCopySource;
    const targetOpen = destinationApi.openMigrationCopyDestination;
    if (failure !== 'destination') {
        vi.spyOn(sourceApi, 'openMigrationCopySource').mockImplementationOnce(async args => {
            const source = await sourceOpen(args);
            const close = source.stream.close;
            return {
                ...source,
                stream: { ...source.stream, close: async () => { await close(); throw new Error('private source close'); } },
            };
        });
    }
    if (failure !== 'source') {
        vi.spyOn(destinationApi, 'openMigrationCopyDestination').mockImplementationOnce(async args => {
            const target = await targetOpen(args);
            return { ...target, close: async () => { await target.close(); throw new Error('private target close'); } };
        });
    }
    await expect(runSqliteMigrationCopy(input)).rejects.toMatchObject({ name: 'MigrationCopyError' });
    expect(remote.run?.state).toBe('completed');
});
...
it.each(['beyond-terminal', 'between-batches', 'missing-domain', 'extra-domain'] as const)(
    'refuses locally claimed %s progress without advancing remote state',
    async fault => {
        const { input } = await prepared(true);
        await runSqliteMigrationCopy(input);
        const read = MigrationManifestStore.prototype.read;
        vi.spyOn(MigrationManifestStore.prototype, 'read').mockImplementationOnce(function (...args) {
            ...
            const checkpoints = current.checkpoints.map(checkpoint => (
                checkpoint.domain === 'session-instructions' && fault === 'beyond-terminal'
                    ? { ...checkpoint, ordinal: 99 }
                    : checkpoint
            ));
            if (fault === 'missing-domain') {
                checkpoints.splice(checkpoints.findIndex(checkpoint => checkpoint.domain === 'session-instructions'), 1);
            }
            if (fault === 'between-batches') {
                const index = checkpoints.findIndex(checkpoint => checkpoint.domain === 'session-instructions');
                checkpoints[index] = { ...checkpoints.find(checkpoint => checkpoint.domain === 'session-instructions')!, ordinal: 0 };
            }
            if (fault === 'extra-domain') {
                checkpoints.push({ ...checkpoints[0]!, domain: 'unknown-domain' });
            }
