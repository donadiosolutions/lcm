  ...
  it("proves the constrained user-systemd path and performs real admission", () => {
    ...
    try {
      ...
      const child = [
        ...
        `import { parseUidMap, hostUidForNamespaceUid } from ${JSON.stringify(`file://${authPath}`)};`,
        ...
        "if (!Number.isSafeInteger(overflow) || parent.uid !== overflow || parent.gid !== overflow || (parent.mode & 0o7777) !== 0o1777) throw new Error('child parent overflow topology is invalid');",
        ...
        "if (hostUidForNamespaceUid(ranges, 0) !== undefined) throw new Error('child root is mapped');",
