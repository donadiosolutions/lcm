...
- CI is plan-driven: the `plan` job selects the test files related to a pull request or merge-queue change and runs them sharded across runners, while pushes to `main`/`release` run the complete suite with coverage and enforce the 100% per-file gate in the `report` job. Keep `scripts/ci-plan.mjs` (graph-invisible inputs, integration triggers) and `scripts/ci-test-shards.mjs` (shard ownership) synchronized with new test inputs and Vitest projects; their contract tests fail otherwise.
