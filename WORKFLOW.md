...
- Before changing external admission, read `docs/external-admission.md`. Preserve fail-closed exact-head CI+DCO admission. A PR that changes CI, build, package, production, or trusted test-harness inputs also requires either an authenticated exact-head Copilot dynamic run or exact Dependabot PR and commit provenance. The write-capable evaluator uses trusted workflow code only and never executes or consumes PR-controlled code, artifacts, or caches.
