# Rebranding Design: lossless-claude → @donadiosolutions/lcm
...
Move the project from personal namespace (`@ipedro/lossless-claude`, `ipedro/lossless-claude`) to the `lossless-claude` GitHub organization and npm scope (`@donadiosolutions/lcm`, `lossless-claude/lcm`). The CLI binary (`lcm`) and domain (`lossless-claude.com`) are unchanged. Version `1.0.0` is reserved for the official launch; `0.7.0` is used to validate the pipeline end-to-end.
...
6. Run `npm deprecate @ipedro/lossless-claude "Package moved to @donadiosolutions/lcm"`
...
| File | Change |
|---|---|
| `package.json` | `name` → `@donadiosolutions/lcm`; `version` → `0.7.0`; `repository.url` → `git+https://github.com/donadiosolutions/lcm.git`; `homepage`, `bugs.url` → `lossless-claude/lcm` |
| `README.md` | npm badge URL, GitHub URLs, npm install command (`npm i @donadiosolutions/lcm`), claude-plugin install instructions — `lossless-claude.com` domain unchanged |
...
1. **Audit user-facing content** — manually verify README install instructions, claude-plugin instructions, and website copy all reflect `@donadiosolutions/lcm` before triggering publish
2. `npm pack --dry-run` — verify package contents, name, and `repository` field
3. Trigger `publish.yml` manually targeting `0.7.0`
4. Confirm `@donadiosolutions/lcm@0.7.0` appears on npm registry
